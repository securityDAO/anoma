...
mod tests {
    ...
    prop_state_machine! {
        ...
        #[test]
        // TODO: There are still some failing cases being discovered, so this
        // test is disabled for now and to be finished after PoS VP refactor.
        //
        // To run this test specifically, you can use:
        //  `cargo test pos_vp_state_machine_test -- --ignored`
        #[ignore]
        /// A `StateMachineTest` implemented on `PosState`
        fn pos_vp_state_machine_test(sequential 1..50 => ConcretePosState);
    }
    ...
    impl ValidPosAction {
        ...
        pub fn apply(self) {
            ...
            for change in changes {
                apply_pos_storage_change(change, &params, current_epoch)
            }
        }
    ...
    impl InvalidPosAction {
        ...
        pub fn apply(self) {
            ...
            for (epoch, changes) in self.changes {
                for change in changes {
                    apply_pos_storage_change(change, &params, epoch);
                }
            }
        }
    }
